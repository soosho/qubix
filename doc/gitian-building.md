...
*Setup instructions for a Gitian build of Qubix Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Qubix
...
- [Building Qubix Core](#building-qubix-core)
...
Clone the git repositories for Qubix Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/raptor3um/qubix
...
in the Qubix Core repository under 'Fetch and create inputs' to install sources which require
...
To build Qubix Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Qubix Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qubix/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/raptor3um/qubix
...
For example:
```bash
URL=https://github.com/crowning-/qubix.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit qubix=${COMMIT} --url qubix=${URL} ../qubix/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qubix=${COMMIT} --url qubix=${URL} ../qubix/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qubix=${COMMIT} --url qubix=${URL} ../qubix/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the qubix git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../qubix/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/qubix
SIGPATH=/some/root/path/qubix-detached-sigs

./bin/gbuild --url qubix=${BTCPATH},signature=${SIGPATH} ../qubix/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qubix-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qubix-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/qubix-osx-build.assert
```
...
[qubix/gitian.sigs](https://github.com/raptor3um/gitian.sigs/) repository, or if that's not possible to create a pull
request.
